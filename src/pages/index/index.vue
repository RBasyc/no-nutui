    ...
    <view class="index-page">
        ...
        <view class="ai-alert-card">
            <view class="ai-alert-header">
                ...
                <view class="header-right">
                    <view v-if="aiAlertData.priority && !aiAlertLoading && !aiAlertRefreshing" class="ai-alert-priority"
                        :class="getPriorityClass(aiAlertData.priority)">
                        <text>{{ aiAlertData.priority }}优先级</text>
                    </view>
                    <!-- 骨架屏优先级标签 -->
                    <view v-if="aiAlertLoading || aiAlertRefreshing" class="skeleton-priority"></view>
                    <!-- 刷新按钮 - 始终显示 -->
                    <view class="refresh-btn-icon" @tap="handleRefreshAlert"
                        :class="{ 'refreshing': aiAlertRefreshing }">
            ...
            <!-- 有AI建议时显示 -->
            <view v-else-if="aiAlertData.summary" class="ai-alert-wrapper">
                <!-- 内容区域 -->
                <view class="ai-alert-content" :class="{ 'expanded': aiAlertExpanded }">
                    <!-- 概况总结 -->
                    ...
                    <view v-if="aiAlertData.topAlerts && aiAlertData.topAlerts.length > 0" class="ai-alerts-list">
                        <view v-for="(alert, index) in aiAlertData.topAlerts" :key="index" class="ai-alert-item">
                    ...
                    <!-- 综合建议（包含行动建议） -->
                    <view v-if="aiAlertData.recommendations && aiAlertData.recommendations.length > 0"
                        class="ai-recommendations">
                        <text class="recommendations-title iconfont icon-tianjia"> 行动建议</text>
                        <view v-for="(rec, index) in aiAlertData.recommendations" :key="index" class="recommendation-item">
                ...
                <!-- 底部模糊渐变和展开按钮 -->
                <view v-if="!aiAlertExpanded" class="ai-alert-fade">
                    <view class="expand-btn" @tap="toggleAiAlertExpanded">
                        <text class="expand-icon" :class="{ 'icon-rotated': aiAlertExpanded }">▼</text>
                        <text class="expand-text">展开</text>
                    </view>
                </view>
                <!-- 展开状态下的收起按钮 -->
                <view v-else class="ai-alert-collapse">
                    <view class="collapse-btn" @tap="toggleAiAlertExpanded">
        ...
        <view class="list-section">
            ...
            <view v-if="expiringItems.length > 0" class="consumable-list">
                <view v-for="(item, index) in expiringItems" :key="index" class="list-item"
                    ...
                    <view v-if="item.days !== null" class="item-expire-wrapper">
                        <text class="item-expire" :class="getExpiryClass(item.days)">
                            {{ item.days }}天
                        </text>
                    </view>
        ...
        <view class="list-section">
            <view class="list-header">
                ...
                <text class="list-more" @tap="handleExportPurchaseList">导出Excel</text>
            </view>
            <!-- 有数据时显示列表 -->
            <view v-if="purchaseItems.length > 0" class="consumable-list">
                <view v-for="(item, index) in purchaseItems" :key="index" class="list-item purchase-item"
                    @tap="handleItemClick(item)">
                    <text class="item-icon iconfont" :class="item.icon"></text>
                    <view class="item-info">
                        <text class="item-name">{{ item.name }}</text>
...
<script setup>
    ...
    const aiAlertExpanded = ref(false)    // 展开/收起状态
    ...
    // 切换AI预警展开/收起状态
    const toggleAiAlertExpanded = () => {
        aiAlertExpanded.value = !aiAlertExpanded.value
    }
</script>
